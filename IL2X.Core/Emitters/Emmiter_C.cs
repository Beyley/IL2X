...
namespace IL2X.Core.Emitters
{
	public sealed class Emmiter_C : Emitter
	{
		...
		protected override void TranslateModule(ModuleJit module, string outputDirectory)
		{
			...
			{
				using var stream = new FileStream(Path.Combine(outputDirectory, "il2x_builtins.h"), FileMode.Create, FileAccess.Write, FileShare.Read);
				...
				WriteLine();
				WriteLine("#ifndef IL2X_BUILTINS_H");
				...
				WriteLine(@"static inline uint64_t IL2X_SafeSubtractUInt64(uint64_t a, uint64_t b)
{
	...
	return a - b;
				...
				WriteLine("#endif");
				WriteLine("#define IL2X_BUILTINS_H 1");
			}
			...
			// write header forward-declare all types
			{
				...
				using (var writer = new StreamWriter(stream))
				{
					...
					WriteLine("#include \"il2x_builtins.h\"");
				}
			}
